...
class JobApplicationsService {
  ...
  async submitApplication(applicationData, jobOfferId) {
    try {
      ...
      const payload = {
        ...
        first_name: applicationData.name,
  ...
  async getApplicationsByEmail(email) {
    try {
      const url = `${import.meta.env.VITE_DIRECTUS_URL}/items/job_applications?filter[email][_eq]=${email}&fields=*,job_offer.id,job_offer.translations.*&sort[]=-application_date`;
  ...
  async hasAlreadyApplied(email, jobOfferId) {
    try {
      const url = `${import.meta.env.VITE_DIRECTUS_URL}/items/job_applications?filter[email][_eq]=${email}&filter[job_offer][_eq]=${jobOfferId}&limit=1`;
